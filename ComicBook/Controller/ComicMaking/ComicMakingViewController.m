...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if ((touch.view == self.view || touch.view == imgvComic) &&
        ((location.y - shinkLimit.y) > [self getShrinkValue] ||
         (location.x - shinkLimit.x) > [self getShrinkValue]) &&
...
- (void)stopDrawing
{
    ...
    CGSize size = CGSizeMake(CGRectGetWidth(imgvComic.frame), CGRectGetHeight(imgvComic.frame));
...
//        UIImageWriteToSavedPhotosAlbum(printScreen, nil, nil, nil);
//        printScreen = [UIImage ScaletoFill:printScreen toSize:CGSizeMake(214, 355)];
        
//        [self.glideScrollView reloadItemsAtIndex:printScreen];
    });
}
