...
//    UIImage* temImage = [self cropImage:image.image toRect:CGRectMake(0, 0, image.image.size.height, image.image.size.width)];
//    [self saveImagetoFolder:temImage FileName:@"temImage.png"];
    ...
    UIBezierPath *aPath;
    UIBezierPath *aPath_WB;
    
    aPath = [UIBezierPath bezierPath];
    aPath_WB = [UIBezierPath bezierPath];
    aPath.miterLimit = -10;
    aPath.flatness = 0;
    
    aPath_WB.miterLimit = -10;
    aPath_WB.flatness = 0;
    ...
    CGPoint p1 = [MZCroppableView convertCGPoint:[[points objectAtIndex:0] CGPointValue] fromRect1:image.frame.size toRect2:image.image.size];
    [aPath moveToPoint:CGPointMake(p1.x, p1.y)];
    [aPath_WB moveToPoint:CGPointMake(p1.x, p1.y)];
...
//        [aPath setLineWidth:10];
        ...
        [aPath_WB addLineToPoint:CGPointMake(p.x, p.y)];
    }
...
//    UIBezierPath *aPath_WB = [aPath copy];
    ...
    {
        aPath = [self smoothedPathWithGranularity_:60 withPath:aPath];
    }
    
    NSLog(@"==9");
    *imgWithOutBorder = [self Sample:aPath_WB image:image];
    
//    [self saveImagetoFolder:*imgWithOutBorder FileName:@"imgWithOutBorder.png"];
...
//    for(UIBezierpath _paths in aPath)
//    {
...
//    [aPath setLineWidth:4.0];
    ...
    [aPath stroke];
    UIImage *whiteflippedImage = [self cropUsingPath:aPath Rect:rect image:image.image];
    whiteflippedImage  = [self ipMaskedImageNamed:whiteflippedImage color:[UIColor whiteColor]];
    [self saveImagetoFolder:whiteflippedImage FileName:@"whiteflippedImage.png"];
//    whiteflippedImage = [self transparentBorderImage:1 withImage:whiteflippedImage];
//    [self saveImagetoFolder:whiteflippedImage FileName:@"whiteflippedImage_1.png"];
    // -----------------
    
    maskedImage = [self drawImage:maskedImage inImage:whiteflippedImage atPoint:CGPointMake(0, 0)];
//    [self saveImagetoFolder:maskedImage FileName:@"maskedImage.png"];
    NSLog(@"==15");
    // self.imgCropedSection = maskedImage;
...
- (UIBezierPath*)smoothedPathWithGranularity_:(NSInteger)granularity withPath:(UIBezierPath *)path;
{
    NSMutableArray *points = [pointsFromBezierPath(path) mutableCopy];
